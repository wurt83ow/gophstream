...
func NewBaseController(ctx context.Context, storage Storage, kafka KafkaProducer, log Log) *BaseController {
...
// @Summary Add message
// @Description Add a new message to the database and send it to Kafka
// @Tags Messages
// @Accept json
// @Produce json
// @Param message body models.RequestMessage true "Message Info"
// @Success 200 {string} string "Message added and sent to Kafka successfully"
...
func (h *BaseController) AddMessage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Info("error marshaling message: ", zap.Error(err))
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	if err := h.kafka.SendMessage("message_topic", messageData); err != nil {
		h.log.Info("error sending message to Kafka: ", zap.Error(err))
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	w.WriteHeader(http.StatusOK)
	if _, err := w.Write([]byte("Message added and sent to Kafka successfully")); err != nil {
		h.log.Info("error writing response: ", zap.Error(err))
	}
	h.log.Info("Message added and sent to Kafka successfully")
}
